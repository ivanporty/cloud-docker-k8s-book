...
> «Я был на конференции, которая была полностью посвящена только YAML. Это была конференция KubeCon (главная конференция Kubernetes)». Келси Хайтауэр в своем твиттере (один из самых заметных проводников и пропагандистов идей Kubernetes)
...
Работа с YAML в Kubernetes стала практически притчей во языцех. Так как мощь и гибкость Kubernetes требуют создания немалого количества файлов YAML и работы с ними, разработчики часто саркастически замечают, что Kubernetes — это и есть редактирование YAML, не самого удобного для человека языка разметки.
