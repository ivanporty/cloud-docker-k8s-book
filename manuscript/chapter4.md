...
* `WORKDIR` — меняет рабочую директорию в файловой системе контейнера. Для образности представьте эту команду в виде обычных `mkdir` и `cd`.
...
* `CMD` (или `ENTRYPOINT`) — команда, которая будет выполняться после запуска контейнера. Основная форма — массив в квадратных скобках, где указываются команда и ее аргументы. Мы просто запустим свой shell-скрипт. Ему требуется параметр, мы его передаем в том же массиве.
...
Обычная проблема при создании образа — копирование бинарного файла с программой или сервисом, несовместимого со стандартами Linux, особенно для таких языков как Go или С++. К примеру, собрав приложение Go на своем ноутбуке Mac, вы не сможете перенести его в контейнер — внутри контейнера действуют стандарты Linux, и ваше приложение не запустится, несмотря на то что среда запуска контейнеров (container runtime) Docker работает на том же самом ноутбуке. 

Лучшее решение в этом случае — сделать компиляцию и сборку (build) приложения частью построения образа image инструкциями Dockerfile. В этом случае все происходит непосредственно внутри операционного ядра контейнера, и полученный образ будет совместим с любыми стандартными средами запуска контейнеров, в том числе с коммерческими провайдерами облаков.

Что же использовать в качестве базового образа? Снова Ubuntu или, может быть, какую-то еще версию Linux, а затем скопировать туда все необходимые для компиляции и сборки языка программирования инструменты? Мы можем вздохнуть с облегчением — основная часть этой работы уже сделана. Упаковка приложений и сервисов в образы контейнеров стала настолько популярна, что все распространенные языки, их основные версии, нужные для работы с ними инструменты уже доступны на открытом репозитории Docker Hub. Нам остается подобрать нужные версию языка и систему сборки и скопировать файлы с кодом своего приложения.
...
Теперь все прошло успешно. Обновите страницу со списком образов, хранящихся в вашей учетной записи, и вы увидите новый образ, только что отправленный в репозиторий.
