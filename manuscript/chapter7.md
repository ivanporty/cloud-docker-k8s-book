...
* Локальный запуск взаимодействующих сервисов Docker Compose. Один вариант - запуск всех микросервисов внутри виртуальной машины Docker, и организация их взаимодействия с помощью оркестратора. Есть множество минусов - необходимо поддерживать дополнительную конфигурацию Compose, недоступны сервисы Kubernetes, а также доступ к библиотекам и функциям API облака, в котором работает приложение. 
...
После развертывания ваш нужно перенаправить порт (5005) на свою локальную машину (`kubectl port-forward …`). Затем ваша среда разработки присоединяется к удаленному процессу и вы сможете наконец отлаживать свой код, работающий в удаленном кластере.
